...
data = aggregate(data[c("opt","noOpt")],list(rate = data$rate),mean)

plot(spline(data$rate, data$opt, method="natural"), type="l", main=sprintf("Wykres zależności czasu przejazdu\nod parametru 'rate'\n(rozmiar sieci drogowej = %s ulic)", args[2]), col="red", xlab="Parametr 'rate' [krok symulacji/pojazd]", ylab="Całkowity czas przejazdu [krok symulacji]")
lines(spline(data$rate, data$noOpt, method="natural"), type="l", col="blue")
...
write("Parametr 'rate' [krok symulacji/pojazd] & Optymalizacja [krok symulacji] & Brak optymalizacji [krok symulacji] \\\\ \\hline", file=tableFile, append=TRUE)
...
write("\\caption{Zależność czasu przejazdu od parametru \\textit{rate}.}", file=tableFile, append=TRUE)
