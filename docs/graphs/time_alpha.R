...
data = aggregate(data[c("opt","noOpt")],list(alpha = data$alpha),mean)

plot(spline(data$alpha, data$opt, method="natural"), type='l', main=sprintf("Wykres zależności czasu przejazdu\nod parametru 'alpha'\n(rozmiar sieci drogowej = %s ulic)", args[2]), col="red", xlab="Parametr 'alpha' [j]", ylab="Całkowity czas przejazdu [krok symulacji]")
lines(spline(data$alpha, data$noOpt, method="natural"), type="l", col="blue")
...
write("Parametr alpha [j] & Optymalizacja [krok symulacji] & Brak optymalizacji [krok symulacji] \\\\ \\hline", file=tableFile, append=TRUE)
...
write("\\caption{Zależność czasu przejazdu od parametru \\textit{alpha}'.}", file=tableFile, append=TRUE)
